...
# Init Kalman
ekf = EKF(frequency=IMU_FPS)
num_samples_kalman=1
# Init start location Quaternion
Q_kalm = np.zeros((num_samples_kalman, 4))
...
cube = Entity(model="cube", texture=bw_texture, scale=3, position=Vec3(-2.5,0,0))
cube_kalm = Entity(model="cube", texture=bw_texture, scale=3, position=Vec3(2.5,0,0))
...
def update():
    ...
    for imuPacket in imuPackets:
        ...
        Q_kalm[i] = ekf.update(Q[1-1], gyr=[-1 * gyroValues.z, -1 * gyroValues.x, gyroValues.y], acc=[-1 * acceleroValues.z, -1 * acceleroValues.x, acceleroValues.y])
        ...
        curr_kalm.setX(Q_kalm[i][2])
        curr_kalm.setI(-Q_kalm[i][0])
        curr_kalm.setJ(Q_kalm[i][1])
        curr_kalm.setK(-Q_kalm[i][3])
        # Set new cube rotation
        cube.set_quat(curr)
        cube_kalm.set_quat(curr_kalm)

# Assign cube update function to Ursina
cube.update = update
cube_kalm.update = update
